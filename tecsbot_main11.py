...
class channel_bot_start(object):
	...
	def main(self):
		...
		def parse_msg(user, msg, irc, user_color, user_disp_name, user_emotes, user_subscriber, user_turbo, user_id, user_type):
			...
			##############
			#problem atm is that when pasting too many of the things over there on the right monitor, nothing happens. But if theres less, but still more than emote_max, it times out and shit. idk i'm tired gnight and do english first
			###################
					...
					if socket_ready[0]:
						...
						if data:
							if data.find("PING") != -1:
								...
								#if you type fast enough, shit screws up
						...
						if self.repeat_on:
							...
							for repeat_set in self.repeat_arr:
								...
								if (current_time - repeat_time >= repeat_interval):
									...
									#we cant check the way we will check with everything else for !ban emote, FUCK how do we do this
									#need to do a seperate check for the ban_emote stuff within the repeat function
									#we could check if the original repeat add had an emote in it, and pass this in the array?
									############################33333
									#need to fix this^ problem as well as make the ban_emotes work
