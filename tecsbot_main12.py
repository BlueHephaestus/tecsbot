...
class channel_bot_start(object):
	...
	def ban_emote_parse(self, user, msg, irc, pre_msg, channel_parsed):
		...
		##################################3333
		#by trying to only display one of them as banned/added, it will make this entire function much more complicated. 
		#for listing and deleting, the offsets will have to be taken into account. Is there another way? Good luck future self.
		########################
						...
						if len(msg_arr) > 3:
							...
							if is_emote(ban_emote, self.emote_arr):
								if ban_emote not in self.ban_emote_arr:#dupes
									if ban_emote in self.special_emote_arr:#bulk adding of specials
										if ban_emote in ['o_O', 'O_o', 'o_o', 'O_O']:
											self.ban_emote_arr.extend(['o_O', 'O_o', 'o_o', 'O_O'])
											send_str = "%s%s added to list of banned emotes.\r\n" % (pre_msg, ban_emote)
										['o_O', 'O_o', 'o_o', 'O_O', ':o', ':O', ':\\', ':/', ':p', ':P', ';p', ';P']
	...
	def main(self):
		...
		def parse_msg(user, msg, irc, user_color, user_disp_name, user_emotes, user_subscriber, user_turbo, user_id, user_type):
			...
			#dont count ourselves in emotes
						...
						if data:
							if data.find("PING") != -1:
								...
								try:
									...
									user_type = '0'
								#if it's a really long wall of emotes
								#we cant do the repeat thing because we don't know who sent it
								#we cant get the previous data, so what are we supposed to let them just send a wall of emotes and follow it with extremely offensive words?
								#I guess this means the user_type to check if a person is mod is out 
						...
						if self.repeat_on:
							...
							for repeat_set in self.repeat_arr:
								...
								if (current_time - repeat_time >= repeat_interval):
									...
									irc.send(send_str)
									#alright so messages in this won't have the convenience of the cap stats for emotes,
									#however they will all be by tecsbot. This means we aren't going to be counting the emotes sent in these
									#messages for !stats emote
									############################33333
									#need to make ban emotes work
...
while True:
	...
	if data != [] and data != '':
		if data.find("PING") != -1:
			...
			irc.send("PONG :tmi.twitch.tv\r\n".encode("utf-8")) #responds to PINGS from the server
			#print "Ponging..."
		#you don't receive anything here
