...
twitch has no limit to how much it can send in each data/packet, so make it search until end of line. Or just implement twisted u fuck
...
def is_mod(user, channel_parsed, user_type):
	...
	for mod in mods_arr:
		...
		try:
			...
			return False
	return False'''
	if user == channel_parsed or user_type != '0':
		return True
	else:
		return False
...
class channel_bot_start(object):
				...
				if user.rstrip() == nick or "Tmi.twitch.tv" in user.capitalize().rstrip() or is_streamer(user, channel_parsed):
					...
					if self.emote_antispam_on:
						...
						for emote in self.emote_arr:
							if msg.count(emote) != 0:
								if emote != "KappaPride":#temporary solution
	...
	def viewers_parse(self, user, msg, irc, pre_msg, channel_parsed):
		...
		if viewers_str == msg:
			...
			if viewer_count != '1':
				...
				send_str = "%sThere is currently %s viewer in the channel.\r\n" % (pre_msg, viewer_count)
			irc.send(send_str)
		else:
			return False
									...
									for ban_emote_variant in self.ban_emote_arr:#special dupes, will need to change this when the bulk/individual checkbox is enabled.
										if isinstance(ban_emote_variant, list):
											if ban_emote in ban_emote_variant:
												send_str = "%s%s variants are already banned.\r\n" % (pre_msg, ban_emote)
												break
									else:
										...
										send_str = "%s%s variants added to list of banned emotes.\r\n" % (pre_msg, ban_emote)
								else:
									if ban_emote not in self.ban_emote_arr:#dupes
										self.ban_emote_arr.append(ban_emote)
										send_str = "%s%s added to list of banned emotes.\r\n" % (pre_msg, ban_emote)
									else:
										send_str = "%s%s is already a banned emote.\r\n" % (pre_msg, ban_emote)
							else:
								send_str = "%s%s is not a valid emote.\r\n" % (pre_msg, ban_emote)
						else:
							send_str = "%sUsage: \"!ban emote add <emote>\"\r\n" % pre_msg
							...
							if is_num(ban_emote):
								...
								if ((len(self.ban_emote_arr)-1) >= ban_emote-1):
									if isinstance(self.ban_emote_arr[ban_emote-1], list):
										send_str = "%sBan emote %s variants removed.\r\n" % (pre_msg, self.ban_emote_arr[ban_emote-1][0])
										'''for ban_emote_variant_index, ban_emote_variant in enumerate(self.ban_emote_arr[ban_emote-1]):
											if ban_emote_variant_index == len(self.ban_emote_arr[ban_emote-1])-1:
												#last element
												send_str += "and \"%s\" removed at index %s.\r\n" % (ban_emote_variant, ban_emote_variant_index)
											else:
												send_str += "\"%s\", " % ban_emote_variant'''
									else:
										send_str = "%sBan emote %s removed at index %s.\r\n" % (pre_msg, self.ban_emote_arr[ban_emote-1], ban_emote)
									del self.ban_emote_arr[ban_emote-1]									
								else:
									send_str = "%sInvalid index for emote removal.\r\n" % pre_msg
							else:
								if ban_emote in self.ban_emote_arr:
									send_str = "%sBan emote %s removed.\r\n" % (pre_msg, ban_emote)									
									...
									send_str = "%sSpecified ban emote does not exist.\r\n" % pre_msg
						else:
							send_str = "%sUsage: \"!ban emote delete/remove <emote/index>\"\r\n" % pre_msg
							
					elif in_front(ban_emote_list_str, msg):
						if len(self.ban_emote_arr) == 0:
							send_str = "%sNo active ban emotes.\r\n" % pre_msg
						else:
							send_str = "%sActive ban emotes: " % pre_msg
							for ban_emote in range(len(self.ban_emote_arr)):
								if (ban_emote != len(self.ban_emote_arr) -1):
									...
									#last element in arr
									if isinstance(self.ban_emote_arr[ban_emote], list):
										next_str = "(%s.) %s variants.\r\n" % (ban_emote+1, self.ban_emote_arr[ban_emote][0])
									else:
										next_str = "(%s.) %s .\r\n" % (ban_emote+1, self.ban_emote_arr[ban_emote])
								send_str = long_print(next_str, send_str, pre_msg, irc)
					elif in_front(ban_emote_clr_str, msg):
						self.ban_emote_arr = []
						send_str = "%sAll ban emotes removed.\r\n" % pre_msg
					elif ban_emote_str == msg:
						send_str = "%sAdd or remove ban emotes to timeout users who say them. Syntax and more information can be found in the documentation.\r\n" % pre_msg
					else:
						send_str = "%sUsage: \"!ban emote add/delete/remove/list/clear\"\r\n" % pre_msg
					irc.send(send_str)
				else:
					send_str = "%sYou have to be a mod to use !ban emote commands.\r\n" % pre_msg
					irc.send(send_str)
			else:
				return False
		else:
	...
	def main(self):
		...
		def parse_msg(user, msg, irc, user_color, user_disp_name, user_emotes, user_subscriber, user_turbo, user_id, user_type):
			...
			if self.repeat_parse(user, msg, irc, pre_msg, channel_parsed) != False:
				return
			...
			'''if user not in viewer_arr:
				...
				irc.send(send_str)'''
			#new followers
			self.follower_arr = new_follower(self.follower_arr, channel_parsed, pre_msg)#will return False if there are none/service unavailable I suppose
			
			#custom commands - these are the exact same as autoreplies. Do we need them regardless?
			#when we add these in they will be the same as autoreplies, but we should just check for a ! point at the beginning and that it isnt' already one of our commands
				#command array for checking if already command?
			##################
			#keep testing this then add it to it's respective function. also dont forget to contact steven if you have the time to gmod with him, he can only play til 8pm
			#if you cant, then remember to ask him tomorrow
			#oh and also archlinux dualboot
			#and chromebooks
			##################
			#commands 
			cmd_str = "!command"
									...
									if len(cmd_msg_arr) == 2:
										...
										for default_cmd in self.default_cmd_arr:#dont add something that is already a default command
											if default_cmd == cmd_phrase:#cant do in front because then !banphrasejaklsdjfk is counted as a default cmd/not allowed
												send_str = "%s%s is already a default command.\r\n" % (pre_msg, cmd_phrase)
												break
										else:
											for cmd_pair in self.cmd_arr:
												if cmd_phrase == cmd_pair[0]:
													#dont add duplicates
													send_str = "%s%s is already an custom command.\r\n" % (pre_msg, cmd_pair[0])
													break
											else:
												cmd_pair = [cmd_phrase, cmd_reply]
												self.cmd_arr.append(cmd_pair)
												send_str = "%sCommand \"%s\" added, with reply \"%s\".\r\n" % (pre_msg, cmd_phrase, cmd_reply)
									else:
										#incorrectly formatted, display usage
										send_str = "%sUsage: \"!command add <command>:<reply>\".\r\n" % pre_msg
								else:
									#incorrectly formatted, display usage
									send_str = "%sUsage: \"!command add <command>:<reply>\".\r\n" % pre_msg
							else:
							...
							if len(msg_arr) == 3:
								...
								if is_num(cmd_phrase):
									if ((len(self.cmd_arr)-1) >= int(cmd_phrase)-1):
										send_str = "%sCommand %s:%s removed at index %s.\r\n" % (pre_msg, self.cmd_arr[int(cmd_phrase)-1][0], self.cmd_arr[int(cmd_phrase)-1][1], cmd_phrase)
										#should be the same index as the pair, after all.
										del self.cmd_arr[int(cmd_phrase)-1]
									else:
										send_str = "%sInvalid index for command removal.\r\n" % pre_msg	
								
								else:
									for cmd_pair in self.cmd_arr:
										if cmd_phrase == cmd_pair[0]:
											send_str = "%sCommand %s:%s removed.\r\n" % (pre_msg, cmd_pair[0], cmd_pair[1])	
											self.cmd_arr.remove(cmd_pair)
											break
									else:
										send_str = "%sSpecified command does not exist.\r\n" % pre_msg
							else:
								#incorrectly formatted, display usage
								send_str = "%sUsage: \"!command delete/remove <command/index>\".\r\n" % pre_msg
						#list commands
						elif cmd_list_str == msg:
							#check to make sure there are commands to list
							if len(self.cmd_arr) == 0:
								send_str = "%sNo active commands.\r\n" % pre_msg
							else:
								send_str = "%sActive commands: " % pre_msg
								for cmd_pair in range(len(self.cmd_arr)):
									...
									if (cmd_pair != len(self.cmd_arr)-1):
										...
										#last element in arr
										next_str = "(%s.) %s: %s.\r\n" % (cmd_pair+1, cmd_phrase, cmd_reply)
									send_str = long_print(next_str, send_str, pre_msg, irc)	
						#clear commands
						elif cmd_clr_str == msg:
							self.cmd_arr = []
							send_str = "%sAll custom commands removed.\r\n" % pre_msg
						#just cmd string, display usage
						elif cmd_str == msg:
							send_str = "%sAdd or remove custom commands. Syntax and more information can be found in the documentation.\r\n" % pre_msg
						else:
							send_str = "%sUsage: !command add/delete/remove/list/clear\r\n" % pre_msg
					else:
						send_str = "%sYou have to be a mod to use !command commands.\r\n" % pre_msg
					irc.send(send_str)
				else:			
					if self.cmd_on:
						for cmd_pair in self.cmd_arr:
				...
				if in_front(emote_stats_str, msg):
					...
					if len(msg_arr) == 2:
						...
						if is_emote(emote, self.emote_arr):
							...
							send_str = "%sInvalid emote for emote stats.\r\n" % pre_msg
					else:
						send_str = "%sUsage: !stats <emote>\r\n" % pre_msg
				#dont count emotes sent by us
				if user != nick:
					for emote in self.emote_arr:
						if emote in msg:
							emote_count = msg.count(emote)
		...
		while True:
			...
			if stream_online(channel_parsed):	
				...
				while True:
					...
					if socket_ready[0]:
						data = irc.recv(2408) #gets output from IRC server
						...
						if data:
							if data.find("PING") != -1:
								...
								print "Ponging..."
							elif in_front(":tmi.twitch.tv CAP * ACK", data):
								print "Connected."
								#send_str = "%sConnected.\r\n" % pre_msg
								#irc.send(send_str)
							else:
								stats_arr = data.split(";", 6)
								#if you type fast enough, it prints all the tags that come before it as well. This likely is caused by data being sent faster than the program can handle it. 
								try :
									user_color = stats_arr[0].split("=")[1]
									if user_color == '':
										user_color = '0'
								except:
									user_color = '0'
								try:
									user_disp_name = stats_arr[1].split("=")[1]
									if user_disp_name == '':
										user_disp_name = '0'
								except:
									user_disp_name = '0'
								try:
									user_emotes = stats_arr[2].split("=")[1]
									if user_emotes == '':
										user_emotes = '0'
								except:
									user_emotes = '0'
								try:
									user_subscriber = stats_arr[3].split("=")[1]
									if user_subscriber == '':
										user_subscriber = '0'
								except:
									user_subscriber = '0'	
								try:
									user_turbo = stats_arr[4].split("=")[1]
									if user_turbo == '':
										user_turbo = '0'
								except:
									user_turbo = '0'
								try:
									user_id = stats_arr[5].split("=")[1]
									if user_id == '':
										user_id = '0'
								except:
									user_id = '0'
								try:
									user_type = stats_arr[6].split(":", 2)[0].split("=")[1]
									if user_type == ' ':
										user_type = '0'
								except:
									user_type = '0'
								
								data_arr = [stats_arr[6].split(":", 2)[1],stats_arr[6].split(":", 2)[2]]
								
								if len(data_arr) == 2:
									user = (data_arr[0].split("!"))[0]
									msg = data_arr[1]
									msg = msg.rstrip()
									#for even printing
									if len(user) < 7:
									...
									'''
									#more debugging stuff- exec time
									#start = time.time()
									parse_msg(user, msg, irc, user_color, user_disp_name, user_emotes, user_subscriber, user_turbo, user_id, user_type)
									#end = time.time() 
									#print end-start
						else:
							print "Reconnecting..."
...
try:
	...
	#thread.start_new_thread(channel_bot_start("#dark_element_slave1").main, ())
