...
replace the moderators list thing with a dictionary of people that speak and user_type = mod <-- !mods /!moderators command
user_type can be mod, admin, global_mod, staff, and blank if nothing 
...
def get_uptime_min(channel_parsed):
	...
	created_at = time.mktime(start_time)
	...
	created_at = now_utc - created_at
	minute = created_at / 60
...
def get_uptime_str(channel_parsed):
	...
	created_at = time.mktime(start_time)
	#convert to unix time so we can calculate amount of hours and seconds it's been up, and other calculations.
	#created_at = (start_time - datetime(1970, 1, 1)).total_seconds()
	#subtract 4 hours from the (now unix)time, making it equal in time zones to ours,
	#then take current time and created time and get the difference.
	now_utc = (datetime.utcnow() - datetime(1970, 1, 1)).total_seconds()
	...
	created_at = now_utc - created_at
	print created_at
	return parse_sec_condensed(created_at)
...
'''we don't use this anymore-just reassign the variables, cna't see myself needing this again
...
class TwitchBot(irc.IRCClient, object):
	...
	def link_whitelist_parse(self, user, msg, channel_parsed):
		...
		if self.link_whitelist_on:
			if in_front(link_whitelist_str, msg):
				if is_mod(user, self.channel_parsed, self.user_type):
					msg_arr = msg.split(" ")
	...
	def spam_permit_parse(self, user, msg, channel_parsed):
		#spam permits
					...
					if len(msg_arr) == 3:
						...
						if is_num(permit_user):
							if ((len(self.permit_arr)-1) >= int(permit_user)-1):
								send_str = "Permit %s removed at index %s." % (self.permit_arr[int(permit_user)-1][1], permit_user)
							...
							for permit_pair in self.permit_arr:
								if permit_user == permit_pair[1]:
									...
									send_str = "Permit \"%s\" removed." % (permit_user)		
									break
							else:
						...
						for permit_pair in range(len(self.permit_arr)):
							if (permit_pair != len(self.permit_arr) -1):
								...
								if self.permit_arr[permit_pair][3] == "time":
									...
									next_str = "(%s.) %s : permanent, " % (permit_pair+1, self.permit_arr[permit_pair][1])
							else:
								#last element in arr
								if self.permit_arr[permit_pair][3] == "time":
									next_str = "(%s.) %s : %s." % (permit_pair+1, self.permit_arr[permit_pair][1], parse_sec_condensed(self.permit_arr[permit_pair][2]))
				...
				if len(msg_arr) == 2:
					...
					for permit_pair in self.permit_arr:
						if permit_pair[1] == permit_user:
							send_str = "%s's spam permit has been removed." % (permit_user)
							self.permit_arr.remove(permit_pair)
							break
				else:
	...
	def antispam_parse(self, user, msg, channel_parsed):
		#antispam
		...
		if self.antispam_on:
			for permit_pair in self.permit_arr:
				...
				if user.rstrip() == self.nickname or is_streamer(user, self.channel_parsed) or is_mod(user, self.channel_parsed, self.user_type):
					return False
				else:
					#if it's not tecsbot
	...
	def banphrase_parse(self, user, msg, channel_parsed):
		#banphrase
	...
	def test_parse(self, user, msg, channel_parsed):
		#test
		test_str = "!test"
	...
	def autoreply_parse(self, user, msg, channel_parsed):
		#autoreplies 
	...
	def set_parse(self, user, msg, channel_parsed):
		#sets			
	...
	def vote_parse(self, user, msg, channel_parsed, vote_option_arr, vote_dict, vote_users, vote_total):
		#voting
		######################################################################################
		...
		if in_front(vote_str, msg) and is_mod(user, self.channel_parsed, self.user_type) and self.vote_on == False:
	...
	def raffle_parse(self, user, msg, channel_parsed):
		#raffle
			...
			if self.raffle_on:
				...
				if start_raffle_str == msg:
					if is_mod(user, self.channel_parsed, self.user_type):
						...
						send_str = "Only mods can start raffles." 
					self.write(send_str)
				elif raffle_str == msg and is_mod(user, self.channel_parsed, self.user_type):
	...
	def roulette_parse(self, user, msg, channel_parsed):
		#roulette
			...
			if in_front(rol_str, msg):
				if rol_str == msg:
					...
					if random.random() < self.rol_chance:
						...
						if is_mod(user, self.channel_parsed, self.user_type) == False:
			...
			if self.ball_on:
				if is_mod(user, self.channel_parsed, self.user_type):
					...
					if in_front(ball_add_str, msg):
						...
						self.write(send_str)
						return
					elif ball_str == msg and is_mod(user, self.channel_parsed, self.user_type):
	...
	def chatters_parse(self, user, msg, channel_parsed):
	...
	def viewers_parse(self, user, msg, channel_parsed):
		#viewers
		viewers_str = "!viewers"
		if viewers_str == msg:
	...
	def commercial_parse(self, user, msg, channel_parsed):
		#commercials
		comm_str = "!commercial"
		if comm_str == msg:
	...
	def ban_emote_parse(self, user, msg, channel_parsed):
		#ban emotes
	...
	def repeat_parse(self, user, msg, channel_parsed):
		#repeat commands
	...
	def custom_command_parse(self, user, msg, channel_parsed):
		#custom commands
		cmd_str = "!command"
				...
				if user != self.nickname:
					for emote in self.emote_arr:
						if word_count(msg, emote) != 0:
							emote_count = word_count(msg, emote)
							self.count_dict[emote][0] += emote_count
				#print_dict_by_key(self.count_dict)
			return False
	...
	def main_parse(self, user, msg, user_type):
		...
		if self.link_whitelist_parse(user, msg, self.channel_parsed) != False:
			return
		
		if self.spam_permit_parse(user, msg, self.channel_parsed) != False:
			return
			
		if self.antispam_parse(user, msg, self.channel_parsed) != False:
			return
		
		if self.banphrase_parse(user, msg, self.channel_parsed) != False:
			return
		
		#test command if bot is in chat, should also give connection stats in the response
		if self.test_parse(user, msg, self.channel_parsed) != False:
			return
		
		if self.autoreply_parse(user, msg, self.channel_parsed) != False:
			return
		
		if self.set_parse(user, msg, self.channel_parsed) != False:
			return
		
		vote_return = self.vote_parse(user, msg, self.channel_parsed, self.vote_option_arr, self.vote_dict, self.vote_users, self.vote_total)
		...
		if self.raffle_parse(user, msg, self.channel_parsed) != False:
			return
			
		if self.roulette_parse(user, msg, self.channel_parsed) != False:
			return
		...
		if self.chatters_parse(user, msg, self.channel_parsed) != False:
			return
			
		if self.viewers_parse(user, msg, self.channel_parsed) != False:
			return
		
		if self.subs_parse(user, msg, self.channel_parsed) != False:
			return
			
		if self.commercial_parse(user, msg, self.channel_parsed) != False:
			return
		
		if self.ban_emote_parse(user, msg, self.channel_parsed) != False:
			return
			
		if self.repeat_parse(user, msg, self.channel_parsed) != False:
			return
		
		if self.custom_command_parse(user, msg, self.channel_parsed) != False:
			return
		...
		'''if self.repeat_on:
			...
			for repeat_set in self.repeat_arr:
				...
				if (current_time - repeat_time >= repeat_interval):
					...
					self.write(send_str)
					main_parse(nick, repeat_cmd, irc, '0', nick.capitalize(), '0', '0', '0', '0', 'mod')
		current_time = time.time()
		if current_time - check_current_time >= online_check_interval:
			check_current_time = time.time()
			if stream_online(self.channel_parsed) == False:
