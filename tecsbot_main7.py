...
class channel_bot_start(object):
		...
		if in_front(permit_str, msg):
			if is_mod(user, channel_parsed):
				...
				if len(msg_arr) == 2:
					...
					#possible future functionality for GUI
	...
	def vote_parse(self, user, msg, irc, pre_msg, channel_parsed, vote_option_arr, vote_dict, vote_users, vote_total):
		...
		#######################
		#hiya, hope you slept well. Finish that homework and then keep testing and improving this voting system -note: only math and econ today, no english due, just work on those unit 2 assignments.
		#good luck, have fun
		######################
					...
					if self.vote_on:
						if is_mod(user, channel_parsed):
							...
							if vote_total != 0:
								...
								send_str += "Total votes: %s\r\n" % vote_total
								irc.send(send_str)
								winner_perc = (float(poll_winner[0][1])/vote_total * 100)
	...
	def 8ball_parse(self, user, msg, irc, pre_msg, channel_parsed):#????
	...
	def ban_emotes_parse(self, user, msg, irc, pre_msg, channel_parsed):
	...
	def main(self):
		...
		if self.permit_arr == []:
			...
			permit_pair = [current_time, permit_nick]
			self.permit_arr.append(permit_pair)
		
		self.link_regex = re.compile(ur'\S+\.\w{2,6}\b(?!\.)', re.MULTILINE)
		...
		def parse_msg(user, msg, irc):
			...
			if 'goldmine' in msg or 'BTC' in msg or 'bitcoin' in msg:
				send_str = "%s/ban %s\r\n" % (pre_msg, user)
				irc.send(send_str)
			if "ert" == msg:
				full_exit()
			...
			if viewers_str == msg:
				...
				viewer_count = viewer_data["streams"]["viewers"]
